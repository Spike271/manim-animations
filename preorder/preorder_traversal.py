...
class PreorderTraversal(Scene):
    ...
    def setup_header(self):
        title = Text("Preorder Traversal", font_size=36, weight=BOLD).to_edge(UP, buff=0.1)
        subtitle = Text("Depth First Search: Root → Left → Right", font_size=20, color=GRAY).next_to(title, DOWN, buff=0.1)
    ...
    def update_stack_display(self, op, value):
        ...
        if op == "push":
            txt = Text(f"pre({value})", font_size=18, color=COLOR_ACTIVE)
            box = Rectangle(
                height=0.4,
                width=1.5,
                color=COLOR_ACTIVE,
