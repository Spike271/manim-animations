...
class InorderTraversal(Scene):
    ...
    def setup_header(self):
        title = Text("Inorder Traversal", font_size=36, weight=BOLD).to_edge(UP, buff=0.1)
        subtitle = Text("Depth First Search: Left → Root → Right", font_size=20, color=GRAY).next_to(title, DOWN, buff=0.1)
        self.play(Write(title), Write(subtitle))

    def setup_code_block(self):
        code_str = """def inorder(node):
    if node is None: 
        return
    inorder(node.left)
    visit(node)
    ...
    def update_stack_display(self, op, value):
        ...
        if op == "push":
            txt = Text(f"in({value})", font_size=18, color=COLOR_ACTIVE)
            box = Rectangle(
                height=0.4,
                width=1.5,
                color=COLOR_ACTIVE,
    ...
    def traverse(self, node):
        ...
        # 3. RECURSE LEFT (Before visiting)
        ...
        # 4. VISIT NODE (Between left and right subtrees)
        ...
        # 5. RECURSE RIGHT (After visiting)
...
class PostorderTraversal(Scene):
    """Bonus: Postorder Traversal for completeness"""
    ...
    def construct(self):
        ...
        root = self.build_tree(tree_data, position=UP * 1.5, width=5.0, layer_height=1.2)
        self.draw_tree_edges(root)

        # Output area
        output_label = Text("Postorder: D, E, B, F, G, C, A", font_size=24, color=COLOR_ACTIVE)
        output_label.to_edge(DOWN, buff=0.8)
    ...
    def build_tree(self, data, position, width, layer_height):
        ...
        val, left_data, right_data = data
        node = TreeNode(val, position)
        self.play(FadeIn(node, scale=0.8), run_time=0.2)
